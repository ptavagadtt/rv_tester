...
public:
  ...
  bool push_a_no_id(const bool& aw, const axi::a_no_id_t& a_no_id, axi::id_t& id) {
    axi::a_t a{a_no_id};
    a.w = aw;
        ...
        if ((w_q_wptr_ - w_q_rptr_) < w_q_max_) {
          ...
          cvm::registry::callbacks.push(
              ...
              [=, this]() {
                ...
                if (data_width_ == 64)
                  _axi_sw_mst::axi_sw_mst_w_8(arg.data.data(), strb.data(), arg.last);
                else if (data_width_ == 512)
                  _axi_sw_mst::axi_sw_mst_w_64(arg.data.data(), strb.data(), arg.last);
                else
